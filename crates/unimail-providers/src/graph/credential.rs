...
pub(super) struct GraphCredentialEnvelopeV1 {
...
impl GraphCredentialEnvelopeV1 {
        ...
        let refresh_token = token
            ...
            .ok_or_else(|| {
                ProviderError::new(
                    ...
                    "graph_refresh_token_missing",
                )
            })?;
        let expires_in = i64::try_from(token.expires_in).map_err(|_| {
            ProviderError::new(ProviderErrorKind::Protocol, "graph_token_expiry_invalid")
        })?;
...
impl fmt::Debug for GraphCredentialEnvelopeV1 {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter
            .debug_struct("GraphCredentialEnvelopeV1")
...
pub(super) struct GraphCredentialManager {
...
impl GraphCredentialManager {
    ...
    pub(super) fn create_reference() -> CredentialRef {
        CredentialRef::new(format!("outlook-oauth-{}", uuid::Uuid::new_v4()))
    }

    pub(super) fn persist(
        &self,
        reference: &CredentialRef,
        envelope: &GraphCredentialEnvelopeV1,
    ) -> ProviderResult<()> {
        let bytes = serde_json::to_vec(envelope).map_err(|_| {
            ProviderError::new(
                ProviderErrorKind::Permanent,
                "graph_credential_encode_failed",
            )
        })?;
        self.store
    ...
    pub(super) fn load(
        &self,
        reference: &CredentialRef,
    ) -> ProviderResult<GraphCredentialEnvelopeV1> {
        let secret = self.store.get(reference).map_err(|_| {
            ProviderError::new(
                ProviderErrorKind::Authentication,
                "graph_credential_read_failed",
            )
        })?;
        ...
        let envelope: GraphCredentialEnvelopeV1 = serde_json::from_slice(secret.expose_secret())
            .map_err(|_| {
                ProviderError::new(
                    ProviderErrorKind::Authentication,
                    "graph_credential_invalid",
                )
            })?;
    ...
    pub(super) async fn access_token(
        ...
        if current.access_token != envelope.access_token
            ...
            || current.scopes != envelope.scopes
        {
    ...
    pub(super) fn delete(&self, reference: &CredentialRef) -> ProviderResult<()> {
        self.store.delete(reference).map_err(|_| {
            ProviderError::new(
                ProviderErrorKind::Permanent,
                "graph_credential_delete_failed",
            )
        })
    }
    ...
    async fn refresh_token(
        &self,
        current: &GraphCredentialEnvelopeV1,
        cancellation: &dyn Cancellation,
    ) -> ProviderResult<GraphCredentialEnvelopeV1> {
        ...
        let token = self
            ...
            .await?;
        GraphCredentialEnvelopeV1::from_token(token, Some(&current.refresh_token))
    }
    ...
    fn refresh_lock(&self, reference: &CredentialRef) -> ProviderResult<Arc<AsyncMutex<()>>> {
        let mut locks = self.refresh_locks.lock().map_err(|_| {
            ProviderError::new(
                ProviderErrorKind::Permanent,
                "graph_refresh_lock_unavailable",
            )
        })?;
        Ok(locks
...
impl fmt::Debug for GraphCredentialManager {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str("GraphCredentialManager([redacted])")
    }
}

fn now_epoch_secs() -> ProviderResult<i64> {
    let duration = SystemTime::now().duration_since(UNIX_EPOCH).map_err(|_| {
        ProviderError::new(ProviderErrorKind::Permanent, "graph_system_time_invalid")
    })?;
    i64::try_from(duration.as_secs())
        .map_err(|_| ProviderError::new(ProviderErrorKind::Permanent, "graph_system_time_invalid"))
}
...
mod tests {
    ...
    use super::{
        ENVELOPE_VERSION, GraphConfig, GraphCredentialEnvelopeV1, GraphCredentialManager,
        GraphHttp, REQUIRED_SCOPES, TokenResponse, now_epoch_secs,
    };
    ...
    async fn manager_with_response(
        ...
        status: &'static str,
        body: &'static str,
    ) -> (GraphCredentialManager, tokio::task::JoinHandle<String>) {
        ...
        let config = GraphConfig::for_test(&format!("http://{address}"));
        let http = GraphHttp::new(config.clone()).expect("HTTP client should initialize");
        (GraphCredentialManager::new(config, store, http), server)
    }
    ...
    async fn envelope_round_trips_and_debug_is_redacted() {
        ...
        let config = GraphConfig::for_test("http://127.0.0.1:1");
        let manager = GraphCredentialManager::new(
            config.clone(),
            store.clone(),
            GraphHttp::new(config).expect("HTTP client should initialize"),
        );
        let reference = CredentialRef::new("graph-oauth-fictional-reference");
        ...
        assert_eq!(manager_debug, "GraphCredentialManager([redacted])");
    }
    ...
    async fn expired_credential_refreshes_and_rotates_refresh_token() {
        ...
        let (manager, server) = manager_with_response(
            store,
            "200 OK",
            r#"{"access_token":"new-access","expires_in":3600,"refresh_token":"new-refresh","scope":"offline_access User.Read Mail.ReadWrite Mail.Send","token_type":"Bearer"}"#,
        )
        .await;
        let reference = CredentialRef::new("graph-oauth-expired");
        manager
    ...
    #[tokio::test]
    async fn refresh_retains_old_refresh_token_when_microsoft_omits_one() {
        let store = Arc::new(TestCredentialStore::default());
        let (manager, server) = manager_with_response(
            store,
            "200 OK",
            r#"{"access_token":"new-access","expires_in":3600,"scope":"offline_access User.Read Mail.ReadWrite Mail.Send","token_type":"Bearer"}"#,
        )
        .await;
        let reference = CredentialRef::new("graph-oauth-retain");
        manager
    ...
    async fn refreshed_token_is_not_returned_when_credential_write_fails() {
        ...
        let (manager, server) = manager_with_response(
            store.clone(),
            "200 OK",
            r#"{"access_token":"unpersisted-access","expires_in":3600,"refresh_token":"unpersisted-refresh","scope":"offline_access User.Read Mail.ReadWrite Mail.Send","token_type":"Bearer"}"#,
        )
        .await;
        let reference = CredentialRef::new("graph-oauth-write-failure");
        manager
            .persist(&reference, &envelope("old-access", "old-refresh", 0))
        ...
        assert_eq!(error.code, "graph_credential_write_failed");
    ...
    async fn concurrent_refresh_is_single_flight_even_when_access_token_is_reused() {
        ...
        let (manager, server) = manager_with_response(
            store,
            "200 OK",
            r#"{"access_token":"same-access","expires_in":3600,"scope":"offline_access User.Read Mail.ReadWrite Mail.Send","token_type":"Bearer"}"#,
        )
        .await;
        let manager = Arc::new(manager);
        let reference = CredentialRef::new("graph-oauth-single-flight");
        manager
    ...
    async fn missing_credential_and_invalid_grant_are_authentication_failures() {
        ...
        .await;
        let missing = CredentialRef::new("graph-oauth-missing");
        ...
        assert_eq!(missing_error.code, "graph_credential_missing");

        let revoked = CredentialRef::new("graph-oauth-revoked");
        manager
        ...
        assert_eq!(grant_error.code, "graph_invalid_grant");
    ...
    fn token_envelope_requires_refresh_token_and_required_scopes() {
        let missing_refresh = GraphCredentialEnvelopeV1::from_token(
            TokenResponse {
        ...
        assert_eq!(missing_refresh.code, "graph_refresh_token_missing");

        let missing_scope = GraphCredentialEnvelopeV1::from_token(
            TokenResponse {
                access_token: "fake-access".to_owned(),
                expires_in: 3600,
        ...
        assert_eq!(missing_scope.code, "graph_required_scope_missing");
        ...
        for token in [
            ...
            let error = GraphCredentialEnvelopeV1::from_token(token, None)
                .expect_err("malformed token response should fail");
            assert_eq!(error.kind, ProviderErrorKind::Protocol);
            assert_eq!(error.code, "graph_token_response_invalid");
        }
    }
}
